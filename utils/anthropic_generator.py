...
class ScriptGenerator:
    ...
    def _generate_mock_script(self, team_name: str, date_str: str, 
        ...
        # Build a simple mock script
        script = f"""
# {team_name} Daily Update - {formatted_date}

Hello, fans! Welcome to the {team_name} Daily Update podcast. I'm your host, and I've got all the latest news and updates for you on this {formatted_date}.
...
"""
        ...
        if games:
            for game in games:
                ...
                if status == "Final":
                    ...
                    script += f"\nYesterday, the {team_name} {result} their game "
                    
                    if home_team == team_name:
                        script += f"against the {away_team}. The final score was {home_score}-{away_score}.\n"
                    else:
                        script += f"against the {home_team}. The final score was {away_score}-{home_score}.\n"
                    ...
                    if notable_performances:
                        ...
                        for perf in notable_performances:
                            ...
                            if name:
                                ...
                                stats = []
                                if hr > 0:
                                    stats.append(f"{hr} home run{'s' if hr > 1 else ''}")
                                if hits > 0:
                                    stats.append(f"{hits} hit{'s' if hits > 1 else ''}")
                                if rbi > 0:
                                    stats.append(f"{rbi} RBI{'s' if rbi > 1 else ''}")
                                
                                script += ", ".join(stats) + "\n"
                else:
                    script += f"\nThe {team_name} {'host' if home_team == team_name else 'visit'} the {home_team if away_team == team_name else away_team} today.\n"
        else:
            script += "\nThe team didn't play yesterday. They'll be back in action soon!\n"
        ...
        if standings:
            ...
            script += f"\n## Current Standings\n\n"
            script += f"The {team_name} currently have a record of {wins}-{losses}, "
            
            if division_rank:
                script += f"putting them in {division_rank} place in their division. "
            
            if games_back == 0:
                script += "They're leading their division! "
            elif games_back > 0:
                script += f"They're {games_back} game{'s' if games_back > 1 else ''} behind the division leader. "
        ...
        # Add outro
        script += f"\n## Closing Thoughts\n\nThat's all for today's {team_name} update. Thanks for listening, and be sure to check back tomorrow for the latest news and updates. This is your host signing off. Go {team_name}!\n"
...
Format the script to be easy to read aloud, with clear paragraphs and natural speech patterns.
"""
        
        return prompt
    
    def generate_and_save_script(self, team_code: str, date: Optional[datetime.date] = None) -> str:
        """Generate and save a script for a team."""
        date = date or datetime.date.today()
        date_str = date.strftime("%Y-%m-%d")
